...
export function RoleSelector({ 
  ...
  // Filter roles based on allowedRoles prop
      ...
      <div ref={dropdownRef} className="relative">
        ...
        <button
          ...
          className={`w-full flex items-center justify-between px-4 py-3 border rounded-lg bg-white transition-all duration-200 
            ${isOpen 
              ? `${borderColor} ring-2 ${ringColor} ring-opacity-50 shadow-md` 
              : `border-gray-300 ${hoverBorderColor} hover:shadow-sm`
            }`}
        >
          ...
          <ChevronDown 
            className={`w-5 h-5 text-gray-500 transition-transform duration-300 ${
              isOpen ? `rotate-180 ${chevronColor}` : ''
            }`} 
          />
        </button>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 transition-all duration-150
                    ${isSelected 
                      ? `${selectedBgColor} border-l-4` 
                      : 'hover:bg-gray-50 border-l-4 border-transparent'
                    }`}
                >
                  <div className={`p-2 rounded-md ${role.bgColor} transition-transform duration-200 ${
                  ...
                  {isSelected && (
                    <div className={`w-2 h-2 rounded-full ${indicatorBgColor} animate-pulse`}></div>
                  )}
                </button>
              );
            })}
