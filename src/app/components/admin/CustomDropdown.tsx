...
export function CustomDropdown({ value, onChange, options, placeholder = 'Select...', label }: CustomDropdownProps) {
  ...
  return (
    <div ref={dropdownRef} className="relative">
          ...
          {selectedOption ? (
            <div>
              ...
              {selectedOption.sublabel && (
                <div className="text-sm text-gray-500 mt-0.5">{selectedOption.sublabel}</div>
              )}
            </div>
          ) : (
            placeholder
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-2 bg-white border-2 border-gray-200 rounded-xl shadow-xl overflow-hidden animate-slideDown">
            ...
            {options.map((option) => (
              ...
              >
                <div className="flex-1">
                  ...
                  {option.sublabel && (
                    <div className={`text-sm mt-0.5 ${option.value === value ? 'text-emerald-600' : 'text-gray-500'}`}>
                      {option.sublabel}
                    </div>
                  )}
                </div>
                {option.value === value && (
                  <Check className="w-5 h-5 text-emerald-600 animate-scaleIn" />
                )}
              </button>
            ))}
          </div>
