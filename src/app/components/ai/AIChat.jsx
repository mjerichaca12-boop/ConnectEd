                  ...
                  <div className="prose prose-sm max-w-none text-gray-800 leading-relaxed space-y-2">
                    <ReactMarkdown
                      ...
                      components={{
                        a: ({ node, href, children, ...props }) => {
                          ...
                          return (
                            <a
                              ...
                              className="text-green-600 hover:text-green-700 underline font-medium underline-offset-2 break-all transition-colors"
                              {...props}
                            >
                              {children}
                            </a>
                        ...
                        table: ({ node, ...props }) => (
                          <div className="overflow-x-auto my-3.5 border border-gray-200 rounded-xl shadow-2xs max-w-full">
                            <table className="min-w-full divide-y divide-gray-200 text-xs text-left" {...props} />
                          </div>
                        ),
                        thead: ({ node, ...props }) => (
                          <thead className="bg-gray-50/90 font-bold text-gray-700 uppercase tracking-wider text-[11px]" {...props} />
                        ),
                        th: ({ node, ...props }) => (
                          <th className="px-3.5 py-2.5 font-semibold text-gray-800 border-b border-gray-200" {...props} />
                        ),
                        td: ({ node, ...props }) => (
                          <td className="px-3.5 py-2.5 text-gray-700 border-b border-gray-100 whitespace-normal" {...props} />
                        ),
                        tr: ({ node, ...props }) => (
                          <tr className="hover:bg-green-50/30 transition-colors odd:bg-white even:bg-slate-50/50" {...props} />
                        ),
                        h1: ({ node, ...props }) => (
                          <h1 className="text-base font-extrabold text-gray-900 mt-4 mb-2 border-b border-gray-100 pb-1 flex items-center gap-1.5" {...props} />
                        ),
                        h2: ({ node, ...props }) => (
                        ...
                        h4: ({ node, ...props }) => (
                          <h4 className="text-xs font-semibold text-gray-600 mt-2 mb-1" {...props} />
                        ),
                        p: ({ node, ...props }) => (
                          <p className="mb-2.5 leading-relaxed text-gray-800" {...props} />
                        ),
                        ul: ({ node, ...props }) => (
                          <ul className="list-disc list-outside ml-5 space-y-1.5 my-2.5 text-gray-800" {...props} />
                        ),
                        ol: ({ node, ...props }) => (
                          <ol className="list-decimal list-outside ml-5 space-y-1.5 my-2.5 text-gray-800" {...props} />
                        ),
                        li: ({ node, ...props }) => (
                        ...
                        blockquote: ({ node, ...props }) => (
                          <blockquote className="border-l-4 border-green-500 pl-3.5 py-2 italic bg-green-50/60 rounded-r-xl my-3 text-gray-700 text-xs font-medium" {...props} />
                        ),
                        strong: ({ node, ...props }) => (
                        ...
                        code: ({ node, inline, className, children, ...props }) => {
                          ...
                          if (inline) {
                            return (
                              <code className="bg-slate-100 text-green-700 font-mono text-[12px] px-1.5 py-0.5 rounded-md border border-slate-200" {...props}>
                                {children}
                              </code>
                            );
                          }
                          return (
                            <div className="my-3.5 border border-slate-800 rounded-xl overflow-hidden bg-slate-900 shadow-xs">
                              {match && (
                                <div className="flex items-center justify-between px-3.5 py-1.5 bg-slate-800 border-b border-slate-700 text-[11px] font-mono text-slate-300 font-semibold uppercase tracking-wider">
                                  <span>{match[1]}</span>
                                </div>
                              )}
                              <div className="p-3.5 overflow-x-auto font-mono text-xs text-slate-100 leading-relaxed">
