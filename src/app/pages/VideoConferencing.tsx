...
export function VideoConferencing() {
  ...
  const filteredMeetings = meetings.filter(meeting =>
    ...
    meeting.teacher.toLowerCase().includes(searchQuery.toLowerCase()) ||
                  ...
                  <div className="absolute bottom-1 left-1 right-1">
                    <div className="flex items-center justify-between text-white">
                      ...
                      <div className="flex items-center gap-0.5">
                        {isHandRaised && (
                          <div className="p-0.5 bg-yellow-500 rounded-full">
            ...
            <div className="w-80 bg-gray-800 border-l border-gray-700 flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-2">
                ...
                {participants.filter(p => p.role === 'Teacher').map((participant) => (
                  <div key={participant.id} className="flex items-center gap-3 p-3 bg-emerald-600/20 rounded-lg border border-emerald-600/30">
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Live Meetings First */}
          {filteredMeetings.filter(m => m.status === 'Ongoing').length > 0 && (
            <div>
              ...
              <div className="space-y-4">
                {filteredMeetings.filter(m => m.status === 'Ongoing').map((meeting) => (
                  <div key={meeting.id} className="bg-white rounded-xl border-2 border-red-200 shadow-md hover:shadow-lg transition-shadow">
                    <div className="p-6">
          ...
          {/* Upcoming Meetings */}
          {filteredMeetings.filter(m => m.status === 'Upcoming').length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Upcoming Meetings</h2>
              <div className="space-y-4">
                {filteredMeetings.filter(m => m.status === 'Upcoming').map((meeting) => (
          ...
          {/* Ended Meetings */}
          {filteredMeetings.filter(m => m.status === 'Ended').length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Past Meetings</h2>
              <div className="space-y-4">
                {filteredMeetings.filter(m => m.status === 'Ended').map((meeting) => (
                  <div key={meeting.id} className="bg-white rounded-xl border border-gray-200 shadow-sm opacity-75">
                    <div className="p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-3">
