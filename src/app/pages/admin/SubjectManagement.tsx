...
export function SubjectManagement() {
  ...
  const [subjectFormData, setSubjectFormData] = useState<SubjectFormData>({
    code: '', name: '', description: '', credits: '', teacher: '', schedule: '', capacity: ''
  });
  ...
  const handleEditSubject = (subject: Subject) => {
    setSelectedSubject(subject);
    setEditFormData({
      code: subject.code, name: subject.name, description: subject.description,
      credits: subject.credits.toString(), teacher: subject.teacher,
      schedule: subject.schedule, capacity: subject.capacity.toString()
    });
    ...
    setTimeout(() => {
      setSubjects(subjects.map(subject =>
        subject.id === selectedSubject.id
          ? { ...subject, code: editFormData.code, name: editFormData.name, description: editFormData.description, credits: Number(editFormData.credits), teacher: editFormData.teacher, schedule: editFormData.schedule, capacity: Number(editFormData.capacity) }
          : subject
      ));
      setIsSubmitting(false);
  ...
  const handleExportToCSV = () => {
    ...
    const rows = filteredSubjects.map(subject => [
      subject.code, subject.name, subject.description, subject.credits.toString(),
      subject.teacher, subject.schedule, subject.capacity.toString(), subject.enrolled.toString()
    ]);
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => handleViewSubject(subject)} className="p-2 hover:bg-blue-50 rounded-lg transition-colors" title="View">
                        <Eye className="w-4 h-4 text-blue-600" />
                      </button>
                      <button onClick={() => handleEditSubject(subject)} className="p-2 hover:bg-emerald-50 rounded-lg transition-colors" title="Edit">
                        <Edit className="w-4 h-4 text-emerald-600" />
                      </button>
                      <button onClick={() => handleDeleteSubject(subject)} className="p-2 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar relative">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
                <button
                  onClick={() => { setShowViewModal(false); handleEditSubject(selectedSubject); }}
