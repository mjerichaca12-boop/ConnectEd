...
export function StudentManagement() {
  ...
  const [studentFormData, setStudentFormData] = useState<StudentFormData>({
    name: '', email: '', phone: '', grade: '', section: '', status: 'Active'
  });
  ...
  const handleConfirmToggleStatus = () => {
    ...
    setTimeout(() => alert(`Student ${action} successfully!`), 100);
  };
  ...
  const handleExportToCSV = () => {
    ...
    const rows = filteredStudents.map(student => [
      student.studentId, student.name, student.email, student.phone,
      student.grade, student.section, student.status,
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredStudents.map((student) => (
                    <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button onClick={() => handleViewStudent(student)} className="p-2 hover:bg-blue-50 rounded-lg transition-colors" title="View">
                            <Eye className="w-4 h-4 text-blue-600" />
                          </button>
                          <button onClick={() => handleEditStudent(student)} className="p-2 hover:bg-emerald-50 rounded-lg transition-colors" title="Edit">
                            <Edit className="w-4 h-4 text-emerald-600" />
                          </button>
                          {student.status === 'Active' ? (
                            <button onClick={() => handleToggleStatus(student)} className="p-2 hover:bg-red-50 rounded-lg transition-colors" title="Deactivate">
                              <Ban className="w-4 h-4 text-red-600" />
                            </button>
                          ) : (
                            <button onClick={() => handleToggleStatus(student)} className="p-2 hover:bg-emerald-50 rounded-lg transition-colors" title="Activate">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <CustomSelect
                      ...
                      onChange={(value: string) => setStudentFormData({ ...studentFormData, status: value as 'Active' | 'Inactive' })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-gray-900">
                    {new Date(selectedStudent.enrollmentDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                </div>
              </div>
              <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
                <button
                  onClick={() => { setShowViewModal(false); handleEditStudent(selectedStudent); }}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl">
            <div className="p-6 flex flex-col items-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                studentToToggle.status === 'Active' ? 'bg-red-100' : 'bg-emerald-100'
              }`}>
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button
                onClick={() => { setShowConfirmModal(false); setStudentToToggle(null); }}
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 text-white rounded-lg transition-colors font-medium ${
                  studentToToggle.status === 'Active' ? 'bg-red-600 hover:bg-red-700' : 'bg-emerald-600 hover:bg-emerald-700'
                }`}
              >
