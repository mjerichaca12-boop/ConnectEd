...
export function StudentManagement() {
  ...
  const [studentFormData, setStudentFormData] = useState<StudentFormData>({ name: '', email: '', phone: '', grade: '', section: '', status: 'Active' });
  const [formErrors, setFormErrors] = useState<FormErrors>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showViewModal, setShowViewModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [selectedStudent, setSelectedStudent] = useState<Student | null>(null);
  const [editFormData, setEditFormData] = useState<StudentFormData>({ name: '', email: '', phone: '', grade: '', section: '', status: 'Active' });
  ...
  const handleViewStudent = (student: Student) => { setSelectedStudent(student); setShowViewModal(true); };
  ...
  const handleToggleStatus = (student: Student) => { setStudentToToggle(student); setShowConfirmModal(true); };
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          {student.status === 'Active'
                            ? <button onClick={() => handleToggleStatus(student)} className="p-2 hover:bg-red-50 rounded-lg transition-colors" title="Deactivate"><Ban className="w-4 h-4 text-red-600" /></button>
                            : <button onClick={() => handleToggleStatus(student)} className="p-2 hover:bg-emerald-50 rounded-lg transition-colors" title="Activate"><CheckCircle className="w-4 h-4 text-emerald-600" /></button>
                          }
              ...
              <form onSubmit={handleAddStudent}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <CustomSelect value={studentFormData.status} onChange={(value: string) => setStudentFormData({ ...studentFormData, status: value as 'Active' | 'Inactive' })} options={[{ value: 'Active', label: 'Active' }, { value: 'Inactive', label: 'Inactive' }]} icon={<User className="w-5 h-5" />} className="min-w-[180px]" />
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar relative">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200">
                <button onClick={() => { setShowViewModal(false); handleEditStudent(selectedStudent); }} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors"><Edit className="w-4 h-4" />Edit Student</button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl">
            ...
            <div className="px-6 pb-6 flex gap-3">
              <button onClick={() => { setShowConfirmModal(false); setStudentToToggle(null); }} className="flex-1 px-4 py-3 text-gray-700 border border-gray-300 hover:bg-gray-50 rounded-lg transition-colors font-medium">Cancel</button>
