...
function TeacherManagement() {
  ...
  const handleAddTeacher = async (event) => {
    ...
    try {
      ...
      if (createdTeacherId) {
        try {
          await supabase.from("profiles").delete().eq("id", createdTeacherId).eq("role", "teacher");
        } catch {
  ...
  const handleUpdateTeacher = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...
        subjects: nextSubjectIds,
        status: normalizeTeacherStatus(editFormData.status)
      };

      const { data, error } = await supabase.from("profiles").update(payload).eq("id", selectedTeacher.id).eq("role", "teacher").select(teacherSelectColumns).single();
        ...
        try {
          ...
          await supabase
            ...
            .update({
              ...
              full_name: selectedTeacher.full_name ?? getTeacherName(selectedTeacher),
  ...
  const handleDeleteTeacher = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("profiles").delete().eq("id", teacherId).eq("role", "teacher");
