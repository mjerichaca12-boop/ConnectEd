...
function InfoRow({ icon, label, value, children }) {
  return (
    <div className="group">
      ...
      <div className="flex items-center gap-4 px-5 py-4 bg-gray-50 rounded-2xl border border-gray-200 hover:border-green-200 transition-colors">
        <span className="text-gray-600 shrink-0">{icon}</span>
        {children || <span className="text-gray-900 text-base">{value || "Not available"}</span>}
      </div>
    </div>
  );
}
...
function TeacherProfile() {
  ...
  useEffect(() => {
    ...
    const sub = supabase?.auth.onAuthStateChange?.((_event, session) => {
      if (!isMounted) return;
      // Only logout if no session AND no fallback user
      const fallbackUser = getFallbackCurrentUser();
      ...
      <main className="flex-1 overflow-y-auto scrollbar-hide lg:pl-64">
        ...
        <div className="p-8 space-y-7">
          ...
          <div className="relative rounded-2xl overflow-hidden">
            ...
            {/* Profile content */}
            <div className="bg-white border border-gray-200 px-10 pb-10">
              <div className="flex items-end gap-8 -mt-18 flex-wrap">
                {/* Avatar */}
                <div className="relative group">
                  <div className="w-36 h-36 rounded-3xl border-4 border-white overflow-hidden bg-gradient-to-br from-green-500 to-teal-600 flex items-center justify-center shrink-0 shadow-2xl">
                    {avatarSource ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8 space-y-7">
              ...
              <InfoRow icon={<Phone className="w-4 h-4" />} label="Phone Number">
                {isEditing ? (
                  ...
                  <span className="text-gray-900 text-sm">{profile.phone || "Not set"}</span>
                )}
              </InfoRow>
