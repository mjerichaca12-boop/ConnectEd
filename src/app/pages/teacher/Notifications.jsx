        ...
        if (supabase && user && user.id) {
          ...
          if (!error && isMounted) {
            setNotifications((data || []).map((n) => ({
              ...
              path: n.type === "announcement" ? "/teacher/announcements" : "/teacher/notifications",
            })));
            setLoading(false);
            return;
          }
            ...
            filtered.map((n) => (
              <div key={n.id} className={`px-4 py-3 flex items-start gap-3 ${!n.isRead ? 'bg-green-50' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2">
                    <button onClick={async () => { await markRead(n.id); navigate(n.path || '/teacher/notifications'); }} className="text-xs text-green-600">Open</button>
                  </div>
                </div>
              </div>
            ))
