...
function TeacherMessages() {
  ...
  />
    ...
    onClick={() => {
      ...
      if (conv.unreadCount > 0) {
        setConversations(
          (prev) => prev.map(
            (c) => c.id === conv.id ? { ...c, unreadCount: 0 } : c
          )
        );
      }
    }}
    className={`p-4 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${selectedConversation?.id === conv.id ? "border-emerald-500" : "border-gray-200"}`}
  >
                        <div className="flex items-center justify-between">
                          <div>
                          ...
                          {conv.unreadCount > 0 && <span className="w-6 h-6 bg-emerald-600 text-white text-xs rounded-full flex items-center justify-center">
                              {conv.unreadCount}
                            </span>}
                        </div>
                      </div>)}
    ...
    onSubmit={(e) => {
      ...
      setConversations(
        (prev) => prev.map(
          (c) => c.id === selectedConversation.id ? { ...c, lastMessage: newText, lastMessageTime: (/* @__PURE__ */ new Date()).toISOString() } : c
        )
      );
      setSelectedConversation(
        (prev) => prev ? { ...prev, lastMessage: newText, lastMessageTime: (/* @__PURE__ */ new Date()).toISOString() } : prev
      );
      setMessageInput("");
    }}
  ...
  >
                  ...
                  </div> : <div className="h-full flex items-center justify-center border border-dashed border-gray-200 rounded-xl p-8 text-center text-sm text-gray-500">
