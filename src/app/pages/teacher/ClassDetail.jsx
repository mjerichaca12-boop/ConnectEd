...
function ClassDetail() {
  ...
  const classAverage = classData.students.length > 0 ? Math.round(classData.students.reduce((sum, s) => sum + s.currentGrade, 0) / classData.students.length) : 0;
  const averageAttendance = classData.students.length > 0 ? Math.round(classData.students.reduce((sum, s) => sum + s.attendance, 0) / classData.students.length) : 0;
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredStudents.map((student) => <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          {student.attendance >= 90 ? <CheckCircle className="w-4 h-4 text-emerald-600" /> : <AlertCircle className="w-4 h-4 text-red-600" />}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
    className={`px-2 py-1 text-xs font-medium rounded-full ${student.status === "Active" ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-700"}`}
  >
                          {student.status}
                        </span>
                      </td>
