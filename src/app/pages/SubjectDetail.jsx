...
function SubjectDetail() {
  ...
  >
    ...
    className={`py-4 border-b-2 font-medium transition-colors ${activeTab === "overview" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-600 hover:text-gray-900"}`}
  >
                  Overview
                </button>
                <button
    onClick={() => setActiveTab("modules")}
    className={`py-4 border-b-2 font-medium transition-colors ${activeTab === "modules" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-600 hover:text-gray-900"}`}
  >
                  Modules & Materials
                </button>
                <button
    onClick={() => setActiveTab("grades")}
    className={`py-4 border-b-2 font-medium transition-colors ${activeTab === "grades" ? "border-emerald-600 text-emerald-600" : "border-transparent text-gray-600 hover:text-gray-900"}`}
  >
                  Grading System
                </button>
              </div>
    ...
    /* Learning Objectives */
                  ...
                  <div>
                    ...
                    <ul className="space-y-3">
                      {subject.objectives.map((objective, index) => <li key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
  ...
  />
    ...
    /* Modules Tab */
              ...
              {activeTab === "modules" && <div className="space-y-4">
                  ...
                  {subject.modules.map((module) => <div
    key={module.id}
    className={`p-4 border rounded-lg transition-all hover:shadow-md ${module.completed ? "bg-emerald-50 border-emerald-200" : "bg-white border-gray-200"}`}
  >
                      <div className="flex items-start gap-4">
                        <div className={`p-3 rounded-lg ${module.completed ? "bg-emerald-100" : "bg-gray-100"}`}>
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              {module.dueDate && <div className="flex items-center gap-2 text-sm text-gray-600">
                                  <Calendar className="w-4 h-4" />
                                  Due: {new Date(module.dueDate).toLocaleDateString()}
                                </div>}
                            </div>
                            {module.completed ? <div className="flex items-center gap-2 text-emerald-600">
                                <CheckCircle className="w-5 h-5" />
                                <span className="text-sm font-medium">Completed</span>
                              </div> : <button className="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-sm">
                                Start
                              </button>}
                          </div>
                        </div>
              ...
              {activeTab === "grades" && <div className="space-y-6">
                  <div>
                    ...
                    <div className="space-y-3">
                      {subject.grading.map((item, index) => <div key={index} className="p-4 bg-gray-50 rounded-lg">
