...
alter table public.teacher_invitation_tokens
  add column if not exists profile_id uuid;
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'teacher_invitation_tokens'
      and column_name = 'profile_id'
  ) then
    execute 'create index if not exists teacher_invitation_tokens_profile_id_idx on public.teacher_invitation_tokens using btree (profile_id)';
  end if;
end $$;
