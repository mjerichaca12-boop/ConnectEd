export const DEPED_SUBJECT_CATEGORIES = [
    "Languages / AP / EsP",
    "Science / Mathematics",
    "MAPEH / EPP / TLE",
] as const;

export type DepedSubjectCategory = typeof DEPED_SUBJECT_CATEGORIES[number];

export interface DepedCategorySettings {
    writtenWorksWeight: number;
    performanceTasksWeight: number;
    writtenWorksEnabled: boolean;
    performanceTasksEnabled: boolean;
}

export const DEPED_DEFAULT_GRADE_SETTINGS: Record<DepedSubjectCategory, DepedCategorySettings> = {
    "Languages / AP / EsP": {
        writtenWorksWeight: 40,
        performanceTasksWeight: 60,
        writtenWorksEnabled: true,
        performanceTasksEnabled: true,
    },
    "Science / Mathematics": {
        writtenWorksWeight: 50,
        performanceTasksWeight: 50,
        writtenWorksEnabled: true,
        performanceTasksEnabled: true,
    },
    "MAPEH / EPP / TLE": {
        writtenWorksWeight: 30,
        performanceTasksWeight: 70,
        writtenWorksEnabled: true,
        performanceTasksEnabled: true,
    },
};

const normalizeText = (value: unknown): string => String(value ?? "").trim().toLowerCase();

export const normalizeSubjectCategory = (value: unknown, subjectName: string = ""): DepedSubjectCategory => {
    const normalized = normalizeText(value);
    const matched = DEPED_SUBJECT_CATEGORIES.find((category) => normalizeText(category) === normalized);
    if (matched) return matched;

    const name = normalizeText(subjectName);
    if (/\b(english|filipino|araling\s*panlipunan|ap\b|esp\b|values|hsp|reading|language|literature|komunikasyon)\b/i.test(name) || /^(english|filipino|araling|esp|values|hsp|reading|language)/i.test(name)) {
        return "Languages / AP / EsP";
    }

    if (/\b(science|mathematics|math|biology|physics|chemistry|calculus|algebra|geometry|statistics)\b/i.test(name) || /^(science|mathematics|math)/i.test(name)) {
        return "Science / Mathematics";
    }

    if (/\b(mapeh|music|arts|pe\b|physical\s*education|health|epp|tle|ict|livelihood)\b/i.test(name) || /^(mapeh|music|arts|pe|physical|health|epp|tle)/i.test(name)) {
        return "MAPEH / EPP / TLE";
    }

    return DEPED_SUBJECT_CATEGORIES[0];
};

export const getDepedCategorySettings = (
    subjectCategory: unknown,
    gradingSettingsByCategory: Record<string, any> = {}
): DepedCategorySettings => {
    const category = normalizeSubjectCategory(subjectCategory);
    const fallback = DEPED_DEFAULT_GRADE_SETTINGS[category] || DEPED_DEFAULT_GRADE_SETTINGS[DEPED_SUBJECT_CATEGORIES[0]];
    const stored = gradingSettingsByCategory?.[category] || {};

    return {
        writtenWorksWeight: Number(stored.writtenWorksWeight ?? fallback.writtenWorksWeight) || 0,
        performanceTasksWeight: Number(stored.performanceTasksWeight ?? fallback.performanceTasksWeight) || 0,
        writtenWorksEnabled: stored.writtenWorksEnabled !== false,
        performanceTasksEnabled: stored.performanceTasksEnabled !== false,
    };
};

export type AssessmentComponent = "writtenWorks" | "performanceTasks";

export const inferAssessmentComponent = (assessment: Record<string, any> = {}): AssessmentComponent => {
    const explicit = normalizeText(
        assessment.gradingComponent ?? assessment.grading_component ?? assessment.component ?? assessment.componentType
    );

    if (explicit.includes("written")) return "writtenWorks";
    if (explicit.includes("performance")) return "performanceTasks";

    const descriptor = normalizeText(
        [assessment.designation, assessment.type, assessment.assessment_type, assessment.assignment_type, assessment.title]
            .filter(Boolean)
            .join(" ")
    );

    if (
        descriptor.includes("activity") ||
        descriptor.includes("seatwork") ||
        descriptor.includes("performance") ||
        descriptor.includes("project") ||
        descriptor.includes("practical")
    ) {
        return "performanceTasks";
    }

    if (
        descriptor.includes("quiz") ||
        descriptor.includes("written") ||
        descriptor.includes("test") ||
        descriptor.includes("exam") ||
        descriptor.includes("assignment") ||
        descriptor.includes("worksheet")
    ) {
        return "writtenWorks";
    }

    return "performanceTasks";
};

export const resolveQuarterFromTerm = (term: unknown): number => {
    const normalized = normalizeText(term);
    if (/\b(q1|quarter\s*1|1st\s*quarter|first\s*quarter|term\s*1)\b/.test(normalized)) return 1;
    if (/\b(q2|quarter\s*2|2nd\s*quarter|second\s*quarter|term\s*2)\b/.test(normalized)) return 2;
    if (/\b(q3|quarter\s*3|3rd\s*quarter|third\s*quarter|term\s*3)\b/.test(normalized)) return 3;
    if (/\b(q4|quarter\s*4|4th\s*quarter|fourth\s*quarter|term\s*4)\b/.test(normalized)) return 4;

    const digitMatch = normalized.match(/\b([1-4])\b/);
    if (digitMatch) return Number(digitMatch[1]);

    return 1;
};

export const isCountedGradeStatus = (status: unknown): boolean => {
    const normalized = normalizeText(status);
    return normalized === "graded" || normalized === "returned" || normalized === "passed" || normalized === "failed";
};

export const clampNumericGrade = (value: unknown, min = 0, max = 100): number => {
    const numeric = Number(value);
    if (Number.isNaN(numeric)) return 0;
    return Math.max(min, Math.min(max, numeric));
};

export const roundTwo = (value: unknown): number => Math.round(Number(value || 0) * 100) / 100;

/**
 * Official DepEd Transmutation Table & Formula
 * Minimum passing score is 60 (initial grade), which transmutes to 75 (transmuted grade).
 * Formula: transmuted = 37.5 + (initialGrade * 0.625)
 */
export const transmuteDepEdQuarterGrade = (initialGrade: number | string): number => {
    const numeric = Number(initialGrade);
    if (!Number.isFinite(numeric) || numeric <= 0) return 0;

    const transmuted = 37.5 + (numeric * 0.625);
    return Math.max(0, Math.min(100, Math.round(transmuted)));
};

export const getGradeRemarks = (grade: number): string => {
    if (grade >= 90) return "Outstanding";
    if (grade >= 85) return "Very Satisfactory";
    if (grade >= 80) return "Satisfactory";
    if (grade >= 75) return "Passed";
    return "Needs Improvement";
};

export interface ComponentSummary {
    rawScore: number;
    highestScore: number;
    percentageScore: number;
    weightedScore: number;
    itemCount: number;
}

export interface QuarterSummary {
    quarter: number;
    weights: DepedCategorySettings;
    writtenWorks: ComponentSummary;
    performanceTasks: ComponentSummary;
    initialGrade: number;
    quarterlyGrade: number;
}

export interface DepedStudentComputation {
    subjectCategory: DepedSubjectCategory;
    quarters: {
        quarter1: QuarterSummary;
        quarter2: QuarterSummary;
        quarter3: QuarterSummary;
        quarter4: QuarterSummary;
    };
    finalGrade: number;
    remarks: string;
}

const createComponentSummary = (): ComponentSummary => ({
    rawScore: 0,
    highestScore: 0,
    percentageScore: 0,
    weightedScore: 0,
    itemCount: 0,
});

export const computeDepEdQuarterSummary = ({
    assessmentItems = [],
    assessmentGradesMap = {},
    assessmentStatusMap = {},
    studentId = "",
    subjectCategory = DEPED_SUBJECT_CATEGORIES[0],
    quarter = 1,
    gradingSettingsByCategory = {},
}: {
    assessmentItems?: any[];
    assessmentGradesMap?: Record<string, any>;
    assessmentStatusMap?: Record<string, any>;
    studentId?: string;
    subjectCategory?: unknown;
    quarter?: number;
    gradingSettingsByCategory?: Record<string, any>;
}): QuarterSummary => {
    const writtenWorks = createComponentSummary();
    const performanceTasks = createComponentSummary();
    const weights = getDepedCategorySettings(subjectCategory, gradingSettingsByCategory);
    const targetQuarter = Number(quarter) || 1;

    assessmentItems.forEach((assessment) => {
        const itemQuarter = resolveQuarterFromTerm(
            assessment.term ?? assessment.quarter ?? assessment.gradingTerm ?? assessment.grading_term
        );
        if (itemQuarter !== targetQuarter) return;

        const assessmentId = String(assessment.id ?? "").trim();
        if (!assessmentId) return;

        // Extract student grade (from nested map or direct assessment grade list)
        let gradeValue = assessmentGradesMap?.[assessmentId]?.[studentId];
        if (gradeValue === undefined && assessmentGradesMap?.[assessmentId] !== undefined && typeof assessmentGradesMap[assessmentId] === 'number') {
            gradeValue = assessmentGradesMap[assessmentId];
        }
        if (gradeValue === undefined || gradeValue === null || gradeValue === "") return;

        const status = assessmentStatusMap?.[assessmentId]?.[studentId] ??
            assessmentGradesMap?.[assessmentId]?.status ??
            assessmentGradesMap?.[assessmentId]?.meta?.[studentId]?.status ??
            "graded";

        if (status && !isCountedGradeStatus(status)) {
            return;
        }

        const componentKey = inferAssessmentComponent(assessment);
        const componentSummary = componentKey === "writtenWorks" ? writtenWorks : performanceTasks;

        const rawScore = clampNumericGrade(gradeValue, 0, 100);
        const highestScore = Math.max(
            0,
            Number(assessment.maxPoints ?? assessment.max_points ?? assessment.total_points ?? 100) || 0
        );

        componentSummary.rawScore += rawScore;
        componentSummary.highestScore += highestScore;
        componentSummary.itemCount += 1;
    });

    const finalizeComponent = (summary: ComponentSummary, weight: number, enabled: boolean) => {
        if (!enabled || summary.highestScore <= 0) {
            summary.percentageScore = 0;
            summary.weightedScore = 0;
            return summary;
        }

        summary.percentageScore = roundTwo((summary.rawScore / summary.highestScore) * 100);
        summary.weightedScore = roundTwo((summary.percentageScore * weight) / 100);
        return summary;
    };

    finalizeComponent(writtenWorks, weights.writtenWorksWeight, weights.writtenWorksEnabled);
    finalizeComponent(performanceTasks, weights.performanceTasksWeight, weights.performanceTasksEnabled);

    const initialGrade = roundTwo(writtenWorks.weightedScore + performanceTasks.weightedScore);
    const quarterlyGrade = transmuteDepEdQuarterGrade(initialGrade);

    return {
        quarter: targetQuarter,
        weights,
        writtenWorks,
        performanceTasks,
        initialGrade,
        quarterlyGrade,
    };
};

export const computeDepEdStudentComputation = ({
    assessmentItems = [],
    assessmentGradesMap = {},
    assessmentStatusMap = {},
    studentId = "",
    subjectCategory = DEPED_SUBJECT_CATEGORIES[0],
    gradingSettingsByCategory = {},
}: {
    assessmentItems?: any[];
    assessmentGradesMap?: Record<string, any>;
    assessmentStatusMap?: Record<string, any>;
    studentId?: string;
    subjectCategory?: unknown;
    gradingSettingsByCategory?: Record<string, any>;
}): DepedStudentComputation => {
    const quarters = {
        quarter1: computeDepEdQuarterSummary({
            assessmentItems,
            assessmentGradesMap,
            assessmentStatusMap,
            studentId,
            subjectCategory,
            quarter: 1,
            gradingSettingsByCategory,
        }),
        quarter2: computeDepEdQuarterSummary({
            assessmentItems,
            assessmentGradesMap,
            assessmentStatusMap,
            studentId,
            subjectCategory,
            quarter: 2,
            gradingSettingsByCategory,
        }),
        quarter3: computeDepEdQuarterSummary({
            assessmentItems,
            assessmentGradesMap,
            assessmentStatusMap,
            studentId,
            subjectCategory,
            quarter: 3,
            gradingSettingsByCategory,
        }),
        quarter4: computeDepEdQuarterSummary({
            assessmentItems,
            assessmentGradesMap,
            assessmentStatusMap,
            studentId,
            subjectCategory,
            quarter: 4,
            gradingSettingsByCategory,
        }),
    };

    const quarterGrades = Object.values(quarters).map((summary) => Number(summary.quarterlyGrade || 0));
    const gradedQuarterCount = quarterGrades.filter((value) => value > 0).length;
    const finalGrade = gradedQuarterCount > 0
        ? roundTwo(quarterGrades.reduce((sum, value) => sum + value, 0) / gradedQuarterCount)
        : 0;

    return {
        subjectCategory: normalizeSubjectCategory(subjectCategory),
        quarters,
        finalGrade,
        remarks: getGradeRemarks(finalGrade),
    };
};

export const serializeDepEdComputation = (computation: unknown): string => {
    try {
        return JSON.stringify(computation || {});
    } catch {
        return "{}";
    }
};
